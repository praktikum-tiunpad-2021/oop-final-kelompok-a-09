...
public class SnakeGame extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			VBox root = new VBox();
			Canvas c = new Canvas(width*pointSize, height*pointSize);
			...
			Scene scene = new Scene(root, width * pointSize, height * pointSize);
	...
	public static void snakeCheck(GraphicsContext gc) {
		...
		if(isNewGame && isGameOver){
			...
			gc.fillRect(0, 0, width*pointSize, height*pointSize);

			gc.setFill(Color.WHITE);
			gc.setFont(new Font("Sans-Serif", 50));
			gc.fillText("SNAKE GAME", 155, height*pointSize/2);
			gc.setFont(new Font("Sans-Serif", 18));
			gc.fillText("Press space to start game", 205, height*pointSize/2+30);
			return;
		}
		...
		if (isGameOver) {
			...
			gc.fillText("Score: " + fruitEaten, 265, height*pointSize/2-60);
			gc.setFont(new Font("Sans-Serif", 50));
			gc.fillText("GAME OVER", 165, height*pointSize/2);
			gc.setFont(new Font("Sans-Serif", 18));
			gc.fillText("Press space to restart game", 195, height*pointSize/2+30);
			return;
		}
		...
		if(running){
			...
			case down:
				snake.get(0).addY(1);;
				if (snake.get(0).getY() > height) { // akan game over jika snake menabrak batas bawah
					isGameOver = true;
					running = false;
				}
				break;
			...
			case right:
				snake.get(0).addX(1);;
				if (snake.get(0).getX() > width) { // akan game over jika snake menabrak batas kanan
					isGameOver = true;
					running = false;
				}
				break;
		...
		if (fruitX == snake.get(0).getX() && fruitY == snake.get(0).getY()) {
		...
		gc.setFill(cc);
		gc.fillOval(fruitX*pointSize, fruitY*pointSize, pointSize, pointSize); // ukuran dan bentuk buah
	...
	// new fruit, memunculkan buah baru secara random
	public static void newFruit() {
		start: while (true) {
			...
			for (Point c : snake) {
				if (c.getX() == fruitX && c.getY() == fruitY) {
					continue start;
				}
			}
